...
import { IpoDetails, McPkgPreview, NewPunch, PoPreview, PunchCategory, PunchOrganization, PunchPreview, PunchSort,  } from './apiTypes';
...
import { LibrayTypes, PunchComment } from '@equinor/procosys-webapp-components/dist/typings/apiTypes';
...
const completionApiService = ({  axios }: CompletionApiServiceProps ) => {
    ...
    const getPunchAttachment = async (
      ...
      const { data } = await axios.get(`PunchItems/${punchGuid}/Attachments/${attachmentGuid}`, config);
      return data;
  };
    ...
    const changeCategory = async( 
      ...
      const { data } = await axios.patch(`PunchItems/${punchGuid}/updateCategory`, {
        category,
        rowVersion
      }, {
          ...headers
    ...
    const deletePunchAttachment = async (
      ...
      await axios.delete(`PunchItems/${punchGuid}/Attachments/${attachmentGuid}`, { 
        headers: {
          "x-plant": `PCS$${plantId}`,
          "Content-Type": "application/json-patch+json",
    ...
    return {  getPunchAttachments , getPunchComments, getPunchItem, getPunchOrganizations, getPunchTypes, getPunchSorts, getPunchPriorities, putUpdatePunch, postPunchAction, getPunchAttachment, getLibraryItems, postPunchAttachment, postPunchComment , changeCategory, postNewPunch, deletePunchAttachment
    };
};
